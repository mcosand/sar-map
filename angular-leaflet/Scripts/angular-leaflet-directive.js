  ...
  angular.module('leaflet-directive').factory('leafletLayerHelpers', [
    ...
    function ($rootScope, $log, leafletHelpers) {
      ...
      var layerTypes = {
          ...
          geoJSON: {
            mustHaveUrlOrData: true,
            createLayer: function (params) {
              var dataArg = angular.isDefined(params.url) ? params.url : params.data;
              if (angular.isDefined(L.geoJson)) {
                return new L.geoJson(dataArg, params.options);
              }
              if (!Helpers.GeoJSONPlugin.isLoaded()) {
                return;
              }
              return new L.TileLayer.GeoJSON(dataArg, params.pluginOptions, params.options);
            }
          },
          wms: {
            mustHaveUrl: true,
      ...
      function isValidLayerType(layerDefinition) {
        ...
        if (layerTypes[layerDefinition.type].mustHaveUrlOrData && (!isString(layerDefinition.url) && !isDefined(layerDefinition.data))) {
          $log.error('[AngularJS - Leaflet] A base layer must have a url or data');
          return false;
        } else {
